...
function ClientEditPage() {
  ...
  if (isError || !client) return <div className="py-6 text-sm text-red-500">Failed to load client.</div>
          ...
          {(field) => (
            <div className="flex-1">
              ...
              <input type="email" className={inputClass} value={field.state.value}
                onChange={(e) => field.handleChange(e.target.value)} placeholder="jane@example.com" />
              <FieldError error={field.state.meta.errors[0]} />
            </div>
          )}
        </form.Field>
        ...
        >
          {(field) => (
            <div className="flex-1">
              ...
              <select className={inputClass} value={field.state.value}
                onChange={(e) => field.handleChange(e.target.value as ServiceType)} onBlur={field.handleBlur}>
      ...
      <form.Field name="funding_source">
        {(field) => (
          <div>
            ...
            <input className={inputClass} value={field.state.value}
              onChange={(e) => field.handleChange(e.target.value)} placeholder="e.g. CLBC, Private" />
          </div>
        )}
      </form.Field>
...
function DangerZone({ clientId }: { clientId: string }) {
  ...
  return (
    <div className="mt-10 mb-10 rounded-lg border border-red-200 p-5">
