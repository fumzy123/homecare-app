        ...
        <form className="flex-1 overflow-y-auto px-8 py-6 space-y-4 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
          ...
          <div className="grid grid-cols-2 gap-3">
            <form.Field name="emergency_contact_phone" validators={{ onBlur: ({ value }) => validatePhone(value, true) }}>
              {(field) => (<div><label className={labelClass}>Phone <span className="text-orange">*</span></label><input className={inputClass} value={field.state.value} onChange={(e) => field.handleChange(formatPhone(e.target.value))} onBlur={field.handleBlur} placeholder="604 555 1234" /><FieldError error={field.state.meta.errors[0]} /></div>)}
            </form.Field>
