...
export default function ProfileScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-cream">
      ...
      >
        {/* ── Page header ───────────────────────────────────────────────── */}
        <View className="flex-row items-end justify-between gap-3.5 px-5 pb-3.5 pt-[18px]">
          <View className="flex-1">
            ...
            <Text className="font-serif text-[32px] leading-[0.98] text-ink">
              {profile.first_name}{' '}
              <Text className="font-serif-italic">{profile.last_name}.</Text>
            </Text>
          </View>
          <Text className="font-mono text-[10px] text-ink underline">Edit</Text>
        </View>
        ...
        <View className="flex-row items-center gap-3.5 px-5 pb-1.5">
          ...
          <View className="flex-1">
            <Text className="font-mono text-[9px] uppercase tracking-[0.08em] text-ink-soft">
        ...
        <View className="px-5">
          <ProfileField label="Full name" value={`${profile.first_name} ${profile.last_name}`} />
        ...
        <View className="px-5">
          ...
          <ProfileField
            ...
            value={profile.employment_type ? EMPLOYMENT_TYPE_LABELS[profile.employment_type] ?? profile.employment_type : null}
          />
          <ProfileField label="Role" value={ROLE_LABELS[profile.role] ?? profile.role} />
        ...
        <View className="flex-row gap-2 px-5 py-1.5">
          ...
          <StatTile
            kicker="Hours MTD"
            value={stats ? String(stats.hours_mtd) : '—'}
          />
          <StatTile
            kicker="Overtime MTD"
        ...
        <View className="px-5">
          {[
            { label: 'Notification preferences', glyph: '›', onPress: undefined },
            { label: 'Send password reset email', glyph: '›', onPress: undefined },
            { label: 'Privacy & data', glyph: '›', onPress: undefined },
          ].map((row) => (
            <Pressable
              key={row.label}
              className="flex-row items-center justify-between border-b border-ink/[0.08] py-3"
            >
              <Text className="text-[13px] text-ink">{row.label}</Text>
              <Text className="font-mono text-sm text-ink-soft">{row.glyph}</Text>
            </Pressable>
          ))}
          <Pressable
            onPress={() => signOut()}
          ...
          >
            <Text className="text-[13px] text-orange">
              {signingOut ? 'Signing out…' : 'Sign out'}
            </Text>
            <Text className="font-mono text-sm text-ink-soft">→</Text>
          </Pressable>
        </View>
      </ScrollView>
    </SafeAreaView>
